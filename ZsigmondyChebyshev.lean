...
theorem count_roots_Tn_sub_one_multiset (n : ℕ) (hn : n > 0) (x : ℝ) :
  ...
  if x = 1 then 1
    ...
    -- By definition of roots_Tn_sub_one_multiset, we know that its count of any element x is equal to the count of x in the multiset of roots of T_n - 1.
    have h_count_eq : Multiset.count x (roots_Tn_sub_one_multiset n) = if x = 1 then 1 else if x = -1 ∧ Even n then 1 else if x ∈ Set.Ioo (-1 : ℝ) 1 ∧ x ∈ roots_Tn_sub_one_multiset n then 2 else 0 := by
      have h_count_1 : Multiset.count 1 (roots_Tn_sub_one_multiset n) = 1 := by
        convert count_cos_eq_one n hn using 1
      have h_count_neg1 : Multiset.count (-1) (roots_Tn_sub_one_multiset n) = if Even n then 1 else 0 := by
        convert count_cos_eq_neg_one n hn using 1
      have h_count_open_interval : ∀ x ∈ Set.Ioo (-1 : ℝ) 1, Multiset.count x (roots_Tn_sub_one_multiset n) = if x ∈ roots_Tn_sub_one_multiset n then 2 else 0 := by
        intros x hx
        have h_count_open_interval_aux : ∀ k j : ℕ, k < n → j < n → Real.cos (2 * k * Real.pi / n) = Real.cos (2 * j * Real.pi / n) → k = j ∨ k = n - j ∨ (k = 0 ∧ j = 0) := by
          exact?;
        have h_count_open_interval_aux : ∀ k : ℕ, k < n → Real.cos (2 * k * Real.pi / n) = x → ∀ j : ℕ, j < n → Real.cos (2 * j * Real.pi / n) = x → j = k ∨ j = n - k := by
          intro k hk hk' j hj hj'; specialize h_count_open_interval_aux j k hj hk; ring_nf at *; aesop;
        split_ifs <;> simp_all +decide [ roots_Tn_sub_one_multiset ];
        obtain ⟨ k, hk₁, hk₂ ⟩ := ‹∃ k < n, Real.cos _ = x›;
        have h_count_open_interval_aux : Multiset.count x (Multiset.map (fun x : ℕ => Real.cos (2 * (x : ℝ) * Real.pi / n)) (Multiset.range n)) = Finset.card (Finset.filter (fun j : ℕ => Real.cos (2 * (j : ℝ) * Real.pi / n) = x) (Finset.range n)) := by
          rw [ Multiset.count_map ];
          congr 1 with j ; simp +decide [ eq_comm, mul_assoc, mul_comm, mul_left_comm ];
        rw [ h_count_open_interval_aux, Finset.card_eq_two ];
        use k, n - k;
        constructor;
        · intro h;
          rw [ eq_tsub_iff_add_eq_of_le hk₁.le ] at h;
          norm_num [ show n = 2 * k by linarith ] at *;
          norm_num [ mul_div_mul_right, hn.ne' ] at hk₂ ; linarith;
        · ext j;
          simp [hk₂];
          apply Iff.intro;
          · exact fun hj => ‹∀ k < n, Real.cos ( 2 * ( k : ℝ ) * Real.pi / n ) = x → ∀ j < n, Real.cos ( 2 * ( j : ℝ ) * Real.pi / n ) = x → j = k ∨ j = n - k› k hk₁ hk₂ j hj.1 hj.2;
          · rintro ( rfl | rfl ) <;> simp_all +decide [ Nat.cast_sub hk₁.le ];
            exact ⟨ Nat.pos_of_ne_zero fun h => by subst h; norm_num at hk₂; linarith, by rw [ ← hk₂ ] ; rw [ ← Real.cos_two_pi_sub ] ; rw [ sub_div' ] <;> ring ; positivity ⟩
      by_cases hx1 : x = 1 <;> by_cases hx2 : x = -1 <;> simp_all +decide;
      by_cases hx3 : -1 < x ∧ x < 1 <;> simp_all +decide;
      split_ifs <;> simp_all +decide [ Multiset.count_eq_zero ];
      contrapose! hx3;
      exact ⟨ lt_of_le_of_ne ( by obtain ⟨ k, hk ⟩ := Multiset.mem_map.mp hx3; exact hk.2.symm ▸ Real.neg_one_le_cos _ ) ( Ne.symm hx2 ), lt_of_le_of_ne ( by obtain ⟨ k, hk ⟩ := Multiset.mem_map.mp hx3; exact hk.2.symm ▸ Real.cos_le_one _ ) hx1 ⟩;
    exact h_count_eq
...
Initial goal:
  ...
  h_div : Q ∣ P
  P_int : ℤ[X]
  hP_int : P = Polynomial.map (Int.castRingHom ℝ) P_int
  Q_int : ℤ[X]
  hQ_int : Q = Polynomial.map (Int.castRingHom ℝ) Q_int
  h_div_int : Polynomial.map (Int.castRingHom ℝ) Q_int ∣ Polynomial.map (Int.castRingHom ℝ) P_int
  ⊢ Function.Injective (⇑(Int.castRingHom ℝ) : ℤ → ℝ)
Remaining goals after safe rules:
  case hf
  P_int Q_int : ℤ[X]
  hQ : IsIntPoly (Polynomial.map (Int.castRingHom ℝ) Q_int)
  hQ_monic : (Polynomial.map (Int.castRingHom ℝ) Q_int).Monic
  hP : IsIntPoly (Polynomial.map (Int.castRingHom ℝ) P_int)
  h_div : Polynomial.map (Int.castRingHom ℝ) Q_int ∣ Polynomial.map (Int.castRingHom ℝ) P_int
  ⊢ Function.Injective fun (x : ℤ) => (↑x : ℝ)
tactic 'aesop' failed, failed to prove the goal after exhaustive search.
Initial goal:
  case hx
  P Q : ℝ[X]
...
lemma IsIntPoly_comp_int_mul_X (P : Polynomial ℝ) (c : ℤ) (hP : IsIntPoly P) : IsIntPoly (P.comp (Polynomial.C (c : ℝ) * Polynomial.X)) := by
  ...
  · norm_num [ Polynomial.coeff_zero_eq_eval_zero ];
    ...
    exact hP 0;
  · norm_num [ mul_pow, Polynomial.coeff_mul ];
    norm_num [ Finset.Nat.sum_antidiagonal_eq_sum_range_succ_mk ];
    norm_num [ Polynomial.coeff_eq_zero_of_natDegree_lt, Finset.sum_range_succ' ];
    split_ifs <;> simp_all +decide [ Polynomial.coeff_zero_eq_eval_zero ];
    obtain ⟨ y, hy ⟩ := hP ( k + 1 ) ; exact ⟨ y * c ^ ( k + 1 ), by push_cast; rw [ hy ] ⟩ ;
